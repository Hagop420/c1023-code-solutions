...
async function readSeveral(): Promise<void> {
  ...
  const msgOne = await read('foo2/bar.html');
  console.log(elapsed(), 'readSeveral2:', msgOne);
  const msgTwo = await read('foo3/bar.html');
  return console.log(elapsed(), 'readSeveral3:', msgTwo);
}
