...
export function Todos() {
  ...
  useEffect(() => {
    const fetchedDataFunc = async () => {
      try {
        const fetchingData = await fetch('/api/todos')
        // err checking
        if (!fetchingData.ok) {
          throw new Error(`Error status of: ${fetchingData.status}`)
        }
        const jsonRes = await fetchingData.json()
        setTodos(jsonRes)
      } catch (err) {
        setError(err)
      } finally {
  ...
  async function addTodo(newTodo: UnsavedTodo) {
    try {
      const fetchUsingPost = await fetch('/api/todos', {
        method: 'POST',
        headers: {
      ...
      //  json response back
      const jsonCONV = await fetchUsingPost.json()
      // setTodos(jsonCONV)
      console.log(jsonCONV)
      //  init an empty array
      const newArr = todos.concat(jsonCONV)
      setTodos(newArr)
  ...
  async function toggleCompleted(todo: Todo) {
    ...
    const putAwayFetched = await fetch(`/api/todos/${todo.todoId}`, {
      method: 'PUT',
      headers: {
        "Content-Type": "application/json"
      },
    ...
    //  json response back
    const jsonCONV = await putAwayFetched.json()
    const newMap = todos.map(todoNew => {
