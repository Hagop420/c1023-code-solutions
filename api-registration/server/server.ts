...
app.post('/api/auth/sign-up', async (req, res, next) => {
  try {
     ...
     */
    const passAwait = await argon2.hash(password)
    //  passing the data into the SQL db
    const sql = `
  INSERT INTO "users" ("username" , "hashedPassword")
    VALUES ($1 ,$2)
    returning *
  `
    const dbState = await db.query(sql, [username, passAwait])

    const rowSetter = dbState.rows[0]
    res.json(rowSetter)
