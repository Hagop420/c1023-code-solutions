...
app.post('/api/auth/sign-in', async (req, res, next) => {
  try {
     ...
     */
    const sqlQueryingOverTable = `SELECT * from users where username = $1`


    const params = [username];

    const result = await db.query<User>(sqlQueryingOverTable, params);
    ...
    // CHECKING IF THE PASSWORD'S ARE NOT ALIKE
    ...
    const tokenCreation = jwt.sign(payload, hashKey)
    res.json({ token: tokenCreation, user: payload })
  } catch (err) {
    next(err);
  }
});
